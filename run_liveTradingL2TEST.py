...
def paper_trade(app, api_thread, contract, preprocessor, model_builder, predictor, time_steps, look_ahead):
    ...
    # Process data
    new_data = app.db.DBtoDF(f"SELECT * FROM {queries.tickdata_date}_l2 WHERE timestamp BETWEEN '{five_minutes_ago}' AND '{now}'")
    ...
    current_price = preprocessor.processed_df['price_0_0'][-1]
...
def main():
    app, api_thread, contract = app_connect(f"{queries.tickdata_date}_l2", 7497, 22)
