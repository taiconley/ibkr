...
def get_realTimeBars(app, contract):
    reqId = 2 # Unique identifier for the request
    barSize = 5  # Bar size in seconds (5 seconds in this case)
    whatToShow = "TRADES"  # Type of data to return, "TRADES", "BID", "ASK", "MIDPOINT", etc.
    useRTH = True  # Use Regular Trading Hours only? 0 means no, 1 means yes
    realtime = 1  # Whether to return real-time data, 1 for real-time, 0 for delayed

    app.reqRealTimeBars(reqId, contract, barSize, whatToShow, useRTH, [])
    #app.reqRealTimeBars(3001, contract, 5, "TRADES", True, [])
...
def main():
    app, api_thread, contract = app_connect(table_date, 7497, 5)
